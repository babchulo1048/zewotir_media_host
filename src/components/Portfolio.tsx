...
import { useState, useEffect, useCallback, useRef } from "react";
// We need Select for the mobile dropdown, assuming it's available in your components/ui
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Card } from "@/components/ui/card";
import {
  Play,
  ...
  ExternalLink,
  Headset,
  Monitor, // New icon for TV Host
  Mic, // New icon for MCing
  Users, // New icon for Interviews
  Loader2,
  AlertTriangle,
} from "lucide-react";
import { Button } from "@/components/ui/button";
...
// const API_BASE_URL = "http://localhost:3001/api/v1/portfolio/assets";
const API_BASE_URL ="https://zewotir-media-host-backend.onrender.com/api/v1/portfolio/assets";

const Portfolio = () => {
  // --- UPDATED: Default tab and state variables ---
  const [activeTab, setActiveTab] = useState("tvhost"); // Set default to new tab
  const [tvHostWork, setTvHostWork] = useState<PortfolioAsset[]>([]); // NEW
  const [mcWork, setMcWork] = useState<PortfolioAsset[]>([]); // NEW
  const [interviewWork, setInterviewWork] = useState<PortfolioAsset[]>([]); // NEW
  // REMOVED: mediaWork, artProjects
  const [voiceoverSamples, setVoiceoverSamples] = useState<PortfolioAsset[]>(
    []
  );
  const [isLoading, setIsLoading] = useState(true);
  ...
  // Define the tab options for the mobile dropdown
  const tabOptions = [
    { value: "tvhost", label: "TV Host" },
    { value: "mcing", label: "MC & Events" },
    { value: "interviews", label: "Strategic Interviews" },
  ...
  const fetchAssets = useCallback(async () => {
    ...
    // UPDATED: New specialized API calls
    const tvHostPromise = fetch(`${API_BASE_URL}/tvhost`).then((res) => {
      if (!res.ok) throw new Error("Failed to fetch TV Host assets.");
      return res.json();
    });
    const mcPromise = fetch(`${API_BASE_URL}/mcing`).then((res) => {
      if (!res.ok) throw new Error("Failed to fetch MCing assets.");
      return res.json();
    });
    const interviewPromise = fetch(`${API_BASE_URL}/interviews`).then((res) => {
      if (!res.ok) throw new Error("Failed to fetch Interview assets.");
      return res.json();
    });
    const voiceoverPromise = fetch(`${API_BASE_URL}/voiceover`).then((res) => {
      if (!res.ok) throw new Error("Failed to fetch Voice-Over assets.");
    ...
    try {
      const [tvHostAssets, mcAssets, interviewAssets, voiceoverAssets] =
        await Promise.all([
          tvHostPromise,
          mcPromise,
          interviewPromise,
      ...
      setTvHostWork(tvHostAssets);
      setMcWork(mcAssets);
      setInterviewWork(interviewAssets);
      setVoiceoverSamples(voiceoverAssets);
      // REMOVED: setMediaWork, setArtProjects
    } catch (err: any) {
      console.error("API Fetch Error:", err);
      setError(
        `Could not load portfolio data. Please check the API server is running on ${API_BASE_URL}. (${err.message})`
  ...
  // --- Helper Component for Voiceover Playback (KEPT THE SAME) ---
  const VoiceoverCardPlayer: React.FC<{ item: PortfolioAsset }> = ({
    item,
  }) => {
    const audioRef = useRef<HTMLAudioElement>(null);
  ...
  // --- GENERIC CARD RENDERER (Updated default text) ---
  ...
  const renderPortfolioCard = (item: PortfolioAsset, type: string) => (
    ...
    >
      ...
      <div className="p-6">
        ...
        <Button asChild variant="secondary" className="w-full text-base">
          <a href={item.link_url} target="_blank" rel="noopener noreferrer">
            {/* Action text updated to be more universal for all these media types */}
            {type.includes("MC") || type.includes("Interview")
              ? "View Project"
              : "Watch Clip"}
            <ExternalLink className="w-4 h-4 ml-2" />
          </a>
  ...
  // --- VOICEOVER CARD RENDERER (KEPT THE SAME) ---
  ...
  const renderContent = () => {
    ...
    return (
      <>
        {/* 1. TV Host Content (New) */}
        <TabsContent value="tvhost" className="animate-fade-in">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {tvHostWork.length > 0 ? (
              tvHostWork.map((item) => renderPortfolioCard(item, "TV Host"))
            ) : (
              <p className="col-span-3 text-center text-muted-foreground py-10">
                No TV Host clips found.
              </p>
            )}
          </div>
        </TabsContent>

        {/* 2. MC & Events Content (New) */}
        <TabsContent value="mcing" className="animate-fade-in">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {mcWork.length > 0 ? (
              mcWork.map((item) => renderPortfolioCard(item, "MC & Events"))
            ) : (
              <p className="col-span-3 text-center text-muted-foreground py-10">
                No MC & Event projects found.
              </p>
            )}
          </div>
        </TabsContent>

        {/* 3. Strategic Interviews Content (New) */}
        <TabsContent value="interviews" className="animate-fade-in">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {interviewWork.length > 0 ? (
              interviewWork.map((item) =>
                renderPortfolioCard(item, "Strategic Interview")
              )
            ) : (
              <p className="col-span-3 text-center text-muted-foreground py-10">
                No Strategic Interview samples found.
              </p>
            )}
          </div>
        </TabsContent>

        {/* 4. VOICE-OVER Content (Intact) */}
        <TabsContent value="voiceover" className="animate-fade-in">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {voiceoverSamples.length > 0 ? (
              voiceoverSamples.map((item) => renderVoiceoverCard(item))
    ...
    <section id="portfolio" className="py-20 md:py-32 bg-secondary/10">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 animate-fade-in">
          ...
          <div className="w-20 h-1 bg-primary mx-auto mb-8"></div>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto font-sans">
            Showcasing my expertise in media, hosting, and strategic
            communication.
          </p>
        </div>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          {/* --- MOBILE/SMALL SCREEN DROPDOWN (New) --- */}
          <div className="sm:hidden w-full max-w-sm mx-auto mb-12">
            <label htmlFor="portfolio-tabs" className="sr-only">
            ...
            <select
              ...
              className="block w-full rounded-lg border border-primary/20 bg-card py-3 px-4 text-base font-semibold text-primary focus:ring-primary focus:border-primary"
            >
          ...
          {/* --- DESKTOP/LARGE SCREEN TABS (Updated) --- */}
          <TabsList className="hidden sm:grid w-full max-w-4xl mx-auto grid-cols-4 mb-12 bg-card border border-primary/20 h-auto p-1 rounded-lg">
            {/* TV Host */}
            <TabsTrigger
              value="tvhost"
              className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground py-3 rounded-md transition-all font-semibold"
